...
# Save normalized counts in nf-core format (without gene annotations, just gene_id and counts)
normalized_counts_nfcore <- as.data.frame(counts(dds_ex_test, normalized = TRUE))
normalized_counts_nfcore <- cbind(gene_id = rownames(normalized_counts_nfcore), normalized_counts_nfcore)
write.table(normalized_counts_nfcore, file=file.path(output_dir, "deseq2_normalized_counts.txt"),  sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)
...
# Save rlog counts in nf-core format
rlog_counts_nfcore <- as.data.frame(assay(vsd))
rlog_counts_nfcore <- cbind(gene_id = rownames(rlog_counts_nfcore), rlog_counts_nfcore)
write.table(rlog_counts_nfcore, file=file.path(output_dir, "deseq2_rlog_counts.txt"), 
            sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)

# Save DESeq2 object
dds_file <- file.path(output_dir, "deseq2.dds.RData")
dds <- dds_ex_test
